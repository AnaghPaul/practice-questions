...
const longer = (element1, element2) => {
...
const getMedian = function (array) {
  ...
  const median = isEven(sortedArray.length)
...
const filterPopularPosts = function (posts) {
  const listOfLikes = posts.map((post) => post.likes);
  const averageLikes = listOfLikes.reduce(sum, 0) / listOfLikes.length;
...
const filterActiveUsersByPostCount = function (users) {
  const listOfPostCount = users.map((user) => user.postCount);
  const averagePosts = listOfPostCount.reduce(sum, 0) / listOfPostCount.length;
...
// console.log(filterByLikes([{id: 1, likes: 100}, {id: 2, likes: 50}], 60))
...
const updateObjectAttribute = function (attribute, updationFn) {
  return function (object) {
    return (object[attribute] = updationFn(object[attribute]));
  };
};
...
const filterLongProductNames = function (products, minLength) {
  const uppercaseNames = products.filter(
    (product) => product.name.length > minLength
  );

  return uppercaseNames.map((product) => ({
    ...product,
    name: product.name.toUpperCase(),
  }));
};

// console.log(filterLongProductNames([{ name: "apple" }, { name: "banana" }], 2));
...
const filterHighPriceWithVAT = function (products, vatRate, threshold) {
  ...
  //   ...product,
  //   pricePostVAT: product.price * (vatRate / 100 + 1),
  // }));
  const productsWithVAT = products.map((product) => ({
    ...product,
    price: product.price * (vatRate / 100 + 1),
  }));

  // return productsWithVAT.filter((product) => product.pricePostVAT > threshold);
  return productsWithVAT.filter((product) => product.price > threshold);
};
...
const filterByCategoryAndPrice = function (products, category, maxPrice) {
  const productsOfACategory = products.filter(
    (product) => product.category.type === category
  );

  return productsOfACategory.filter((product) => product.price <= maxPrice);
};
