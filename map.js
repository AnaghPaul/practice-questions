...
const incrementTargetCount = function (target) {
  return function (count, element) {
...
const getFrequencyChart = function (array) {
  return function (target) {
    return { name: target, count: countOccurences(target, array) };
  };
};
...
const doubleLetterString = function (string) {
...
const vowelCount = function (string) {
  const incrementIfVowel = incrementIfTrue(isVowel, 1);
...
// console.log(reversedWordsOf(['bye world', 'hi darkness']));

// extract unique characters from ["apple", "banana", "grape"] => ["aple", "ban", "grape"]
// Maintain the order of their first appearance in each string
const uniqueCharactersOf = function (strings) {
  return strings.map((string) => removeDuplicates(string).join(""));
};
...
const sortedLettersOf = function (strings) {
  return strings.map((string) => [...string].toSorted().join(""));
};
...
const extractNames = function (objects) {
  // return objects.map((person) => person.name);
...
const percentageContributions = function (numbers) {
  ...
  return numbers.map(
    (countribution) => (countribution / totalContribution) * 100
  );
};
